      ...
      <div class="flex flex-col gap-4 p-6">
        <div class="grid grid-cols-3 gap-8 mt-8">
          <div v-for="hairstyle in hairstyles" :key="hairstyle.id" class="p-4 bg-white rounded-lg shadow-md">
            <img :src="hairstyle.image" :alt="hairstyle.name" class="object-cover mb-4 rounded-lg h-60 w-96">
            <div class="flex flex-col items-center">
              <span class="text-lg font-semibold">{{ hairstyle.name }}</span>
              <span>{{ hairstyle.price }} FCFA</span>
              <span>{{ hairstyle.nb_employe }} employés</span>
              <div class="flex justify-between w-full mt-4">
                <router-link :to="{ name: 'UpdateCoiffure', params: { id_coiffure: hairstyle.id_hairstyle_type } }">
                  <button class="px-4 py-2 mr-2 font-bold text-white bg-blue-500 rounded hover:bg-blue-700">Modifier</button>
                </router-link>
                <button type="button" @click="confirmDelete(hairstyle.id_hairstyle_type)" class="px-4 py-2 font-semibold text-white bg-red-500 rounded hover:bg-red-600">Supprimer</button>
              </div>
            </div>
          </div>
        </div>
...
onMounted(async () => {
  try {
    ...
    toast.error("Erreur lors du chargement des coiffures", { timeout: 5000 });
  }
});
...
const onDelete = async (id) => {
  try {
    ...
    hairstyles.value = hairstyles.value.filter(hairstyle => hairstyle.id_hairstyle_type !== id);
  } catch (error) {
